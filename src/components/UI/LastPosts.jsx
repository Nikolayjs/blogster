  ...
  return (
    <section className="px-2 lg:flex lg:space-x-2">
      ...
      >
        ...
        <div className="p-4 absolute bottom-0 left-0 z-20">
          ...
          <div className="flex mt-3">
            ...
            />
            <div>
              <p className="font-semibold text-gray-200 text-sm">{posts.at(-1).user.fullName}</p>
              <time className="text-gray-400 text-xs">{getDate(posts.at(-1).createdAt)}</time>
            </div>
          </div>
        </div>
      </Link>
      <Link
        ...
        to={`/posts/${posts.at(-2)._id}`}
      >
        <div className="absolute left-0 top-0 w-full h-full z-10 bg-gradient-to-b from-black/10 to-black/70 hover:from-black/10 hover:to-black/80"></div>
        <img
          className="absolute left-0 top-0 w-full h-full z-0 object-cover"
          src={posts.at(-2).imageUrl}
          alt="1"
        />
        <div className="p-4 absolute bottom-0 left-0 z-20">
          <h2 className="text-4xl font-semibold text-gray-100 leading-tight">
          ...
          <div className="flex mt-3">
            ...
            />
            <div>
              <p className="font-semibold text-gray-200 text-sm">{posts.at(-2).user.fullName}</p>
              <time className="text-gray-400 text-xs">{getDate(posts.at(-2).createdAt)}</time>
            </div>
          </div>
        </div>
      </Link>
